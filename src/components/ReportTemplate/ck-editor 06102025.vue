    ...
    updateRows(block, recalculate = false) {
      ...
      for (let i = 0; i < totalRows; i++) {
        ...
        const row = block.headers.map((h, cIndex) => {
          if (dataIndex < 0) {
            if (h.field === "__custom__" && h.format.includes("{{nama}}")) {
              const key = `${i}-${cIndex}`;
              if (!block.manualInputs) block.manualInputs = {};
              if (!(key in block.manualInputs)) block.manualInputs[key] = "";

              const recordWithManual = {
                ...record,
                peranan: block.manualInputs[key] || ""
              };

              return {
                type: "input",
                key,
                value: block.manualInputs[key],
                extraText: this.applyCustomFormat(h.format, recordWithManual)
              };
            }
          ...
          if (h.field === "__custom__" && h.format) {
            return { type: "text", value: this.applyCustomFormat(h.format, record) };
          }

          if (h.field && this.variableJson[h.field] !== undefined) {
            return { type: "text", value: this.variableJson[h.field][dataIndex] || "" };
          }
      ...
      const blocksWithRowCount = this.blocks.map((block) => {
        if (block.type === "table") {
          return {
            ...
            manualInputs: block.manualInputs || {}
          };
        }
        return block;
      });
