<template>
    <Layout>
        <PageHeader title="Lookup Gabungan Parti" pageTitle="Pentadbir" />
        ...
        <BModal v-model="showModal" hide-footer scrollable centered size="lg" hide-header>
            <div class="p-4 rounded">
                <div class="modal-header">
                    <h4 class="mb-3"><strong>Tambah Gabungan Parti</strong></h4>
                ...
                <div class="modal-body">
                    <form>
                        <div class="form-group row align-items-center mb-1" v-for="(entry, index) in data" :key="index">
                            ...
                            <div class="col-1">
                                <input type="color" class="form-control form-control-color" v-model="color[index]" />
                            </div>
                            <div class="col-7">
                                <input type="text" class="form-control" v-model="data[index]"
                                    :placeholder="'Gabungan Parti ' + (index + 1)" />
                            </div>
                            <div class="col-2">
                                <input type="text" class="form-control" v-model="singkatan[index]"
...
export default {
    ...
    data() {
        return {
            headers: [
                ...
                { name: 'Warna', align: 'center', bold: true, width: 10, editable: true, warna: true },
                { name: 'Status', align: 'center', bold: true, width: 10, editable: true },
            ],
            TableLabel: 'Senarai Gabungan Parti',
            tableData: [],
            data: [''],
            singkatan: [''],
            color: ['#000000'], // ✅ Keep track of colors
            showModal: false,
    ...
    methods: {
        add() {
            this.data.push('');
            this.singkatan.push('');
            this.color.push('#000000'); // ✅ Add new color entry
        },
        remove(index) {
            this.data.splice(index, 1);
            this.singkatan.splice(index, 1);
            this.color.splice(index, 1); // ✅ Remove color as well
        },
        openModal() {
            this.showModal = true;
            this.editIndex = null;
            ...
            try {
                ...
                this.tableData = rawData.map(item => ({
                    fields: [
                        ...
                        { value: item.nama },
                        { value: item.color || '#000000', type: 'color' }, // ✅ Load color from API
                        {
        ...
        async simpan() {
            try {
                // Prepare structured data with color included
                const entries = this.data.map((nama, index) => ({
                    nama,
                    singkatan: this.singkatan[index] || nama,
                    color: this.color[index] || '#000000', // ✅ Include color
                }));

                const payload = { data: entries };
                const response = await lookupService.tambahPartiGabungan(payload);
                ...
                newData.forEach(item => {
                    updatedTable.push({
                        fields: [
                            ...
                            { value: item.singkatan },
                            { value: item.nama },
                            { value: item.color || '#000000', type: 'color' }, // ✅ Show new color in table
                            { value: item.aktif == 1 ? 'Aktif' : 'Tidak Aktif' },
                        ]
                    });
                });

                this.successMessage = "✅ Gabungan Parti berjaya disimpan!";
        ...
        async kemaskini({ row }) {
            ...
            const singkatan = row.fields[1].value;
            const nama = row.fields[2].value;
            const color = row.fields[3].value;
            const aktif = row.fields[4].value === 'Aktif' ? 1 : 0;

            try {
                await lookupService.kemaskiniPartiGabungan(id, {
                    nama,
                    singkatan,
                    color,
                    aktif
        ...
        async hapus({ id }) {
            ...
            this.confirmAction = async () => {
                try {
                    await lookupService.hapusPartiGabungan(id);
        ...
        async openRecoverModal() {
            try {
                const response = await lookupService.getDeletedPartiGabungan();
        ...
        async pulihkan() {
            try {
                ...
                await lookupService.pulihPartiGabungan({ ids: this.selectedRecovery });
        ...
        async confirmHapusKekal() {
            try {
                await lookupService.hapusKekalPartiGabungan({ ids: this.selectedRecovery });
