<template>
    <Layout>
        <PageHeader title="Senarai Templat Pelantikan" pageTitle="Pentadbir" />
...
export default {
    ...
    data() {
        return {
            ...
            topicLabel: "Senarai Template Pelantikan",
            // Define headers for the data table
            headers: ["Bil", "Pelantikan", "Tindakan"],
            // Define fields for the data table
            fields: ["index", "pelantikan", "actions"],
            // Define column alignments    
            columnAlignments: ["center", "left", "center"],
            columnWidths: ["5%", "85%", "10%"],
            // Rename Button tambah
            buttonLabel: "Tambah Senarai",
            link: "/pentadbir/template-senarai/template",
    ...
    methods: {
        async loadData() {
            this.loading = true;
            try {
                const response = await templateSenaraiService.getSenaraiTemplate({
                    page: this.page,
        ...
        goToPapar(item) {
            this.$router.push({ path: `/pentadbir/template-senarai/papar`, query: { pelantikan: item.pelantikan } });
        },
        goToKemaskini(item, mode) {
            this.$router.push({ path: `/pentadbir/template-senarai/template`, query: { mod: mode, id: item.id } });
        },
        hapus(item) {
            // console.log('Hapus:', item)
            this.deleteTarget = item.pelantikan;
            this.targetID = item.id;
            this.showConfirmDelete = true;
        },
        async confirmDelete() {
            try {
                await templateSenaraiService.deleteTemplate(this.targetID);
