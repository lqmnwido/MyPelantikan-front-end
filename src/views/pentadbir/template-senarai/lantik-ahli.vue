<template>
    <Layout>
        <PageHeader :title="pageTitle" pageTitle="Pelantikan" />
        <FormWizard :steps="wizardSteps" :formData="form" :topicLabel="topicLabel" @submit="handleFormSubmit"
            :mode="isEditMode">
            <template #default="{ stepIndex }">
                <div v-if="stepIndex === 0">
                    <LayoutForm :leftFields="leftTab" :rightFields="rightTab" v-model:form="form" />
                </div>
            </template>
        </FormWizard>
        <Loader :loading="loading" class="loading-overlay" />

        <SweetAlert :show="alert.success" type="success" title="Berjaya!" text="Calon telah dilantik."
            @closed="alert.success = false" />

        <SweetAlert :show="alert.kemaskini" type="success" title="Berjaya!" text="Calon telah dikemaskini."
            @closed="alert.success = false" />

        <SweetAlert :show="alert.error" type="error" title="Ralat" :text="alert.errorMessage"
            @closed="alert.error = false" />

    </Layout>
</template>

<script>
import Layout from "@/layouts/main.vue";
import PageHeader from "@/components/page-header.vue";
import FormWizard from "@/components/FormWizard.vue";
import LayoutForm from "@/components/LayoutForm.vue";
import { lookupService } from "@/services/lookup.service.js";
import { templateSenaraiService } from "@/services/templateSenarai.service.js";
import SweetAlert from "@/components/SweetAlert.vue";
import Loader from "@/components/widgets/loader";
// function formDataToJson(formData) {
//     const obj = {};
//     for (let [key, value] of formData.entries()) {
//         obj[key] = value;
//     }
//     return obj;
// }

export default {
    components: { Layout, Loader, PageHeader, FormWizard, LayoutForm, SweetAlert },
    name: "TambahPelantikan",
    data() {
        return {
            isEditMode: "add",
            wizardSteps: [
                { title: "Maklumat Lantikan" }
            ],
            form: {
                jawatan: "",
                wakil: "",
                tarikh_lanjutan: "",
                tarikh_lantik: "",
                tarikh_tamat: "",
                catatan: "",
            },

            jawatanList: [],
            //inputGroup Label for perkhidmatan
            inputGroupLabelPerkhidmatan: "Perkhidmatan",

            //4th Tab Fields
            leftTab: [
                { label: 'Tarikh Lantik', type: 'date', model: 'tarikh_lantik', required: true },
            ],
            rightTab: [
                { label: 'Tarikh Tamat', type: 'date', model: 'tarikh_tamat', required: true },
            ],
            loading: false,
            alert: {
                success: false,
                error: false,
                kemaskini: false,
                errorMessage: '',
            },
            topicLabel: "Butiran Ahli",

            templateName: "",
            headerVariable: [],
        };
    },
    async mounted() {

        this.templateName = this.$route.query.pelantikan ? this.$route.query.pelantikan : "";

        await this.loadJenisPelantikan();
        await this.loadJawatan();
        if (this.$route.query.kemaskini === 'true') {
            this.isEditMode = 'update';
            const id = this.$route.query.id;
            if (id) {
                await this.loadAhli(id);
            }
        }
    },
    methods: {
        async loadJenisPelantikan() {
            this.loading = true;
            try {
                const response = await lookupService.getJenisPelantikan();
                this.selectedPelantikan = response.find(item => item.nama === this.templateName) || null;
                this.id_jenis_pelantikan = this.selectedPelantikan?.id || null;
                this.form.id_jenis_pelantikan = this.id_jenis_pelantikan;
            } catch (err) {
                console.error("Failed to load Jenis Pelantikan options:", err);
                this.alert.error = true;
                this.alert.errorMessage = err?.response?.data?.message || err.message || 'Ralat tidak diketahui';
            } finally {
                this.loading = false;
            }
        },
        async loadJawatan() {
            this.loading = true;
            try {
                const response = await lookupService.getJawatan();
                const res = await templateSenaraiService.getTemplateByName(this.templateName);

                for (const header of res[0].headers) {
                    if (header.variable) {
                        this.headerVariable.push(header.variable);
                    }
                }

                this.jawatanList = response
                    .filter(item => item.id_lkp_jenis_pelantikan === this.id_jenis_pelantikan)
                    .sort((a, b) => a.nama.localeCompare(b.nama));

                // Check condition: hide field if no jawatan OR only "Ahli"
                const hasJawatan = this.jawatanList.length > 0;

                // // Build leftTab based on condition
                // this.leftTab = [];

                if (hasJawatan) {
                    this.leftTab.push({
                        label: 'Jawatan',
                        type: 'select-search',
                        placeholder: '- SILA PILIH -',
                        model: 'jawatan',
                        required: true,
                        options: this.jawatanList.map(item => ({
                            label: item.nama,
                            value: item.nama
                        }))
                    });
                }

                if (this.headerVariable.includes("penggal")) {
                    // Always include these fields
                    if (hasJawatan) {
                        this.rightTab.push(
                            {
                                label: 'Penggal', type: 'select', placeholder: '- PILIH PENGGAL -', model: 'penggal', options: [
                                    { label: 'Pertama', value: 1 },
                                    { label: 'Kedua', value: 2 },
                                    { label: 'Ketiga', value: 3 },
                                    { label: 'Keempat', value: 4 },
                                    { label: 'Kelima', value: 5 },
                                    { label: 'Keenam', value: 6 },
                                    { label: 'Ketujuh', value: 7 },
                                    { label: 'Kelapan', value: 8 },
                                    { label: 'Kesimbilan', value: 9 },
                                    { label: 'Kesepuluh', value: 10 },
                                ],
                                required: true,
                            }
                        );
                    } else {
                        this.leftTab.push(
                            {
                                label: 'Penggal', type: 'select', placeholder: '- PILIH PENGGAL -', model: 'penggal', options: [
                                    { label: 'Pertama', value: 1 },
                                    { label: 'Kedua', value: 2 },
                                    { label: 'Ketiga', value: 3 },
                                    { label: 'Keempat', value: 4 },
                                    { label: 'Kelima', value: 5 },
                                    { label: 'Keenam', value: 6 },
                                    { label: 'Ketujuh', value: 7 },
                                    { label: 'Kelapan', value: 8 },
                                    { label: 'Kesimbilan', value: 9 },
                                    { label: 'Kesepuluh', value: 10 },
                                ],
                                required: true,
                            }
                        );
                    }
                }

                if (this.headerVariable.includes("wakil")) {
                    if (hasJawatan) {
                        this.rightTab.push(
                            { label: 'Wakil', type: 'text', placeholder: 'Isikan wakil jika ada...', model: 'wakil' },
                        );
                    } else {
                        this.leftTab.push(
                            { label: 'Wakil', type: 'text', placeholder: 'Isikan wakil jika ada...', model: 'wakil' },
                        );
                    }
                }

                this.leftTab.push({ label: 'Catatan', type: 'textarea', model: 'catatan', rows: 3 });

            } catch (error) {
                console.error("Error loading jawatan:", error);
                this.alert.error = true;
                this.alert.errorMessage = error?.response?.data?.message || error.message || 'Ralat tidak diketahui';
            } finally {
                this.loading = false;
            }
        },
        async handleFormSubmit() {
            try {
                const id = this.$route.query.id;

                const data = this.form;
                const formData = new FormData();

                for (let [key, value] of formData.entries()) {
                    if (value === "null" || value === "undefined" || value === null || value === undefined) {
                        formData.set(key, "");
                    }
                }
                formData.append('id', id);
                formData.append('jawatan', data.jawatan);
                formData.append('kategori', this.templateName);

                formData.append('wakil', data.wakil);
                formData.append('catatan', data.catatan);


                if (this.headerVariable.includes("penggal")) {
                    formData.append('penggal', data.penggal);
                } else {
                    formData.append('penggal', 99999);
                }

                const tarikh_lantik = data.tarikh_lantik
                    ? new Date(data.tarikh_lantik).toISOString().split('T')[0]
                    : '';

                const tarikh_tamat = data.tarikh_tamat
                    ? new Date(data.tarikh_tamat).toISOString().split('T')[0]
                    : '';

                formData.append('tarikh_lantik', tarikh_lantik);
                formData.append('tarikh_tamat', tarikh_tamat);

                formData.append('id_jenis_pelantikan', data.id_jenis_pelantikan);
                // console.log("✅ FormData ready to send:", formDataToJson(formData));

                const id_pelantikan = this.$route.query.id;

                if (this.isEditMode === 'update' && id_pelantikan) {
                    await templateSenaraiService.kemaskiniAhli(id_pelantikan, formData);
                    this.alert.kemaskini = true;
                } else {
                    await templateSenaraiService.lantikAhli(formData);
                    this.alert.success = true;
                }

                this.$router.push({ path: "/rekod-pelantikan/senarai-pelantikan/kategori/papar", query: { pelantikan: this.templateName } });
            } catch (error) {
                console.error("Error preparing form data:", error);
                this.alert.error = true
                this.alert.errorMessage = error?.response?.data?.message || error.message || 'Ralat tidak diketahui'
                return;
            }
        },
        async loadAhli(id) {
            try {
                const res = await templateSenaraiService.maklumatAhli(id);
                const data = res.data.data;

                // Populate your form here
                this.form = {
                    jawatan: data.jawatan,
                    wakil: data.wakil,
                    tarikh_lantik: data.tarikh_lantik,
                    tarikh_tamat: data.tarikh_tamat,
                    penggal: data.penggal,
                    id_jenis_pelantikan: data.id_lkp_jenis_pelantikan,
                    catatan: data.catatan,
                };

            } catch (err) {
                console.error("❌ Error loading profil for edit:", err);
                this.alert.error = true
                this.alert.errorMessage = err?.response?.data?.message || err.message || 'Ralat tidak diketahui'
            }
        },
    },
    computed: {
        pageTitle() {
            return this.isEditMode === 'update' ? 'Kemaskini Suruhanjaya Perkhidmatan Awam' : 'Lantik '.concat(this.templateName ? this.templateName : '');
        },
    }


};
</script>

<style scoped>
.loading-overlay {
    position: absolute;
    top: 0;
    left: 0;
    right: 0;
    bottom: 0;
    z-index: 999;
    width: 100%;
    background: rgba(255, 255, 255, 0.6);
}
</style>