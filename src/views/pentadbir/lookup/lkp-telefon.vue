<template>
    <Layout>
        <PageHeader title="Lookup Telefon" pageTitle="Pentadbir" />
        ...
        <BModal v-model="showModal" hide-footer scrollable centered size="lg" hide-header>
            <div class="p-4 rounded">
                <div class="modal-header">
                    <h4 class="mb-3"><strong>Tambah Telefon</strong></h4>
    ...
    data() {
        return {
            headers: [
                ...
                { name: 'Jenis', align: 'left', bold: true, width: 70, editable: true },
                { name: 'Status', align: 'center', bold: true, width: 10, editable: true },
            ],
            TableLabel: 'Senarai Telefon',
            tableData: [],
            data: [''],
            showModal: false,
    ...
    methods: {
        ...
        async loadData() {
            try {
                ...
                const rawData = await lookupService.getTelefon(); // already .data
                // console.log("📦 Received Telefon:", rawData);
        ...
        async simpan() {
            try {
                ...
                const response = await lookupService.tambahTelefon(payload);

                this.showModal = false;
                this.data = ['']; // Reset input

                // Append new data manually, avoid DOM issue
                const newData = response.data?.Telefon || [];
                ...
                this.successMessage = "✅ Telefon berjaya disimpan!";
        ...
        async kemaskini({ row }) {
            ...
            try {
                await lookupService.kemaskiniTelefon(id, { nama, aktif });
        ...
        async hapus({ id }) {
            ...
            this.confirmAction = async () => {
                try {
                    await lookupService.hapusTelefon(id);
        ...
        async openRecoverModal() {
            try {
                const response = await lookupService.getDeletedTelefon(); // You must create this endpoint
        ...
        async pulihkan() {
            try {
                ...
                await lookupService.pulihTelefon({ ids: this.selectedRecovery });
