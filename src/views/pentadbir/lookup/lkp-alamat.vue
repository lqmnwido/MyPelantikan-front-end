<template>
    <Layout>
        <PageHeader title="Lookup Alamat" pageTitle="Pentadbir" />
        ...
        <BModal v-model="showModal" hide-footer scrollable centered size="lg" hide-header>
            <div class="p-4 rounded">
                <div class="modal-header">
                    <h4 class="mb-3"><strong>Tambah Alamat</strong></h4>
    ...
    methods: {
        ...
        async loadData() {
            try {
                ...
                const rawData = await lookupService.getAlamat(); // already .data
                // console.log("📦 Received Alamat:", rawData);
        ...
        async simpan() {
            try {
                ...
                const response = await lookupService.tambahAlamat(payload);

                this.showModal = false;
                this.data = ['']; // Reset input

                // Append new data manually, avoid DOM issue
                const newData = response.data?.Alamat || [];
                ...
                this.successMessage = "✅ Alamat berjaya disimpan!";
        ...
        async kemaskini({ row }) {
            ...
            try {
                await lookupService.kemaskiniAlamat(id, { nama, aktif });
        ...
        async hapus({ id }) {
            ...
            this.confirmAction = async () => {
                try {
                    await lookupService.hapusAlamat(id);
        ...
        async openRecoverModal() {
            try {
                const response = await lookupService.getDeletedAlamat(); // You must create this endpoint
        ...
        async pulihkan() {
            try {
                ...
                await lookupService.pulihAlamat({ ids: this.selectedRecovery });
