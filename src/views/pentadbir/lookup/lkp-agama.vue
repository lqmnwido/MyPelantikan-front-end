<template>
    <Layout>
        <PageHeader title="Lookup Agama" pageTitle="Pentadbir" />
        ...
        <BModal v-model="showModal" hide-footer scrollable centered size="lg" hide-header>
            <div class="p-4 rounded">
                <div class="modal-header">
                    <h4 class="mb-3"><strong>Tambah Agama</strong></h4>
    ...
    data() {
        return {
            headers: [
                ...
                { name: 'Agama', align: 'left', bold: true, width: 55, editable: true },
                { name: 'Status', align: 'center', bold: true, width: 10, editable: true },
            ],
            TableLabel: 'Senarai Agama',
            tableData: [],
            data: [''],
            showModal: false,
    ...
    methods: {
        ...
        async loadData() {
            try {
                ...
                const rawData = await lookupService.getAgama(); // already .data
                // console.log("📦 Received Agama:", rawData);
        ...
        async simpan() {
            try {
                ...
                const response = await lookupService.tambahAgama(payload);

                this.showModal = false;
                this.data = ['']; // Reset input

                // Append new data manually, avoid DOM issue
                const newData = response.data?.Agama || [];
                ...
                this.successMessage = "✅ Agama berjaya disimpan!";
        ...
        async kemaskini({ row }) {
            ...
            try {
                await lookupService.kemaskiniAgama(id, { nama, aktif });
        ...
        async hapus({ id }) {
            ...
            this.confirmAction = async () => {
                try {
                    await lookupService.hapusAgama(id);
        ...
        async openRecoverModal() {
            try {
                const response = await lookupService.getDeletedAgama(); // You must create this endpoint
        ...
        async pulihkan() {
            try {
                ...
                await lookupService.pulihAgama({ ids: this.selectedRecovery });
        ...
        async confirmHapusKekal() {
            try {
                await lookupService.hapusKekalAgama({ ids: this.selectedRecovery });
