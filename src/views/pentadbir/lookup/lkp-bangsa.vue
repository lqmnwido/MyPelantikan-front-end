<template>
    <Layout>
        <PageHeader title="Lookup Bangsa" pageTitle="Pentadbir" />
        ...
        <BModal v-model="showModal" hide-footer scrollable centered size="lg" hide-header>
            <div class="p-4 rounded">
                <div class="modal-header">
                    <h4 class="mb-3"><strong>Tambah Bangsa</strong></h4>
    ...
    data() {
        return {
            headers: [
                ...
                { name: 'Bangsa', align: 'left', bold: true, width: 55, editable: true },
                { name: 'Status', align: 'center', bold: true, width: 10, editable: true },
            ],
            TableLabel: 'Senarai Bangsa',
            tableData: [],
            data: [''],
            showModal: false,
    ...
    methods: {
        ...
        async loadData() {
            try {
                ...
                const rawData = await lookupService.getBangsa(); // already .data
                // console.log("📦 Received Bangsa:", rawData);
        ...
        async simpan() {
            try {
                ...
                const response = await lookupService.tambahBangsa(payload);

                this.showModal = false;
                this.data = ['']; // Reset input

                // Append new data manually, avoid DOM issue
                const newData = response.data?.Bangsa || [];
                ...
                this.successMessage = "✅ Bangsa berjaya disimpan!";
        ...
        async kemaskini({ row }) {
            ...
            try {
                await lookupService.kemaskiniBangsa(id, { nama, aktif });
        ...
        async hapus({ id }) {
            ...
            this.confirmAction = async () => {
                try {
                    await lookupService.hapusBangsa(id);
        ...
        async openRecoverModal() {
            try {
                const response = await lookupService.getDeletedBangsa(); // You must create this endpoint
        ...
        async pulihkan() {
            try {
                ...
                await lookupService.pulihBangsa({ ids: this.selectedRecovery });
        ...
        async confirmHapusKekal() {
            try {
                await lookupService.hapusKekalBangsa({ ids: this.selectedRecovery });
