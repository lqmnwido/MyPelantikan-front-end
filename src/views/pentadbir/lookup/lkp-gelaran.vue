<template>
    <Layout>
        <PageHeader title="Lookup Gelaran" pageTitle="Pentadbir" />
        <EditableTable :headers="headers" :data="tableData" :TableLabel="TableLabel" @add-row="openModal"
            ...
            @kekananan-changed="handleKekanananUpdate" />

        <BModal v-model="showModal" hide-footer scrollable centered size="lg" hide-header>
            <div class="p-4 rounded">
                <div class="modal-header">
                    <h4 class="mb-3"><strong>Tambah Gelaran</strong></h4>
...
export default {
    ...
    methods: {
        ...
        async loadData() {
            try {
                const rawData = await lookupService.getGelaran();
        ...
        async simpan() {
            try {
                ...
                const entries = this.data.map((nama) => ({
                    nama,
                    singkatan: nama
                }));

                const payload = { data: entries };
                const response = await lookupService.tambahGelaran(payload);
                ...
                this.successMessage = "✅ Gelaran berjaya disimpan!";
        ...
        async _updateGelaran(row) {
            ...
            return lookupService.kemaskiniGelaran(id, {
                nama,
                kekananan,
                aktif
            });
        ...
        async hapus({ id }) {
            ...
            this.confirmAction = async () => {
                try {
                    await lookupService.hapusGelaran(id);
        ...
        async openRecoverModal() {
            try {
                const response = await lookupService.getDeletedGelaran(); // You must create this endpoint
        ...
        async pulihkan() {
            try {
                ...
                await lookupService.pulihGelaran({ ids: this.selectedRecovery });
        ...
        async handleKekanananUpdate({ index, newValue }) {
            this.tableData[index].fields[2].value = parseInt(newValue);
            // This method no longer re-orders the table visually.
            // The user will save the row manually, and the table will
            // reflect the new order after it reloads the data from the server.
        },
        toggleSelectAll() {
            if (this.selectAll) {
        ...
        async confirmHapusKekal() {
            try {
                await lookupService.hapusKekalGelaran({ ids: this.selectedRecovery });
