<template>
    <Layout>
        <PageHeader title="Lookup Status Kahwin" pageTitle="Pentadbir" />
        ...
        <BModal v-model="showModal" hide-footer scrollable centered size="lg" hide-header>
            <div class="p-4 rounded">
                <div class="modal-header">
                    <h4 class="mb-3"><strong>Tambah Status Kahwin</strong></h4>
                        ...
                        <div class="form-group row align-items-center mb-1" v-for="(entry, index) in data" :key="index">
                            ...
                            <div class="col-10">
                                <input type="text" class="form-control" v-model="data[index]"
                                    :placeholder="'Status Kahwin ' + (index + 1)" />
                            </div>
...
export default {
    ...
    data() {
        return {
            headers: [
                ...
                { name: 'Jenis', align: 'left', bold: true, width: 55, editable: true },
                { name: 'Status', align: 'center', bold: true, width: 10, editable: true },
            ],
            TableLabel: 'Senarai Status Kahwin',
            tableData: [],
            data: [''],
            showModal: false,
    ...
    methods: {
        ...
        async loadData() {
            try {
                // const response = await lookupService.getJantina();
                const rawData = await lookupService.getStatusKahwin(); // already .data
                // console.log("📦 Received Status Kahwin:", rawData);
        ...
        async simpan() {
            try {
                ...
                const response = await lookupService.tambahStatusKahwin(payload);

                this.showModal = false;
                this.data = ['']; // Reset input

                // Append new data manually, avoid DOM issue
                const newData = response.data?.StatusKahwin || [];
                ...
                // alert("✅ Jantina berjaya disimpan!");
                this.successMessage = "✅ Status Kahwin berjaya disimpan!";
        ...
        async kemaskini({ row }) {
            ...
            try {
                await lookupService.kemaskiniStatusKahwin(id, { nama, aktif });
        ...
        async hapus({ id }) {
            ...
            this.confirmAction = async () => {
                try {
                    await lookupService.hapusStatusKahwin(id);
        ...
        async openRecoverModal() {
            try {
                const response = await lookupService.getDeletedStatusKahwin(); // You must create this endpoint
        ...
        async pulihkan() {
            try {
                ...
                await lookupService.pulihStatusKahwin({ ids: this.selectedRecovery });
        ...
        async confirmHapusKekal() {
            try {
                await lookupService.hapusKekalStatusKahwin({ ids: this.selectedRecovery });
