<template>
    <Layout>
        <PageHeader title="Lookup Bandar" pageTitle="Pentadbir" />
        ...
        <BModal v-model="showModal" hide-footer scrollable centered size="lg" hide-header>
            <div class="p-4 rounded">
                <div class="modal-header">
                    <h4 class="mb-3"><strong>Tambah Bandar</strong></h4>
                ...
                <div class="modal-body">
                    <form>
                        <div class="form-group row align-items-center mb-1" v-for="(entry, index) in data" :key="index">
                            ...
                            <div class="col-6">
                                <input type="text" class="form-control" style="height:42px;" v-model="data[index].nama"
                                    :placeholder="'Bandar ' + (index + 1)" />
                            </div>
                            <div class="col-4">
                                <Multiselect v-if="negeriOptions.length" v-model="data[index].id_lkp_negeri"
                                    :options="negeriOptions" :searchable="true" :object="true" :close-on-select="true"
    ...
    data() {
        return {
            headers: [
                ...
                { name: 'Bandar', align: 'left', bold: true, width: 35, editable: true },
                { name: 'Negeri', align: 'center', bold: true, width: 25, editable: true },
                { name: 'Status', align: 'center', bold: true, width: 10, editable: true },
            ],
            TableLabel: 'Senarai Bandar',
            tableData: [],
            data: [{ nama: '', id_lkp_negeri: '' }],
            showModal: false,
            ...
            negeriList: [{ id: null, nama: 'Memuat...' }],
        }
    },
    methods: {
        add() {
            // this.data.push('');
            this.data.push({ nama: '', id_lkp_negeri: '' });
        },
        remove(index) {
            this.data.splice(index, 1);
        },
        openModal() {
            this.data = [{ nama: '', id_lkp_negeri: '' }]; // Reset form
            this.showModal = true;
        },
        async loadData() {
            try {
                // const response = await lookupService.getBandar();
                const rawData = await lookupService.getBandar(); // already .data
                // console.log("📦 Received Bandar:", rawData);
                ...
                this.tableData = rawData.map(item => ({
                    fields: [
                        ...
                        { value: item.nama },
                        {
                            value: this.negeriOptions.find(n => n.value === item.id_lkp_negeri)?.label || '—',
                            type: 'select',
                            options: this.negeriOptions.map(n => ({
            ...
            try {
                const payload = {
                    data: this.data.map(entry => ({
                        nama: entry.nama,
                        id_lkp_negeri: entry.id_lkp_negeri?.value || null,
                    }))
                };
                const response = await lookupService.tambahBandar(payload);

                this.showModal = false;
                this.data = [{ nama: '', id_lkp_negeri: '' }]; // Reset input

                // Append new data manually, avoid DOM issue
                const newData = response.data?.Bandar || [];
                ...
                // alert("✅ Bandar berjaya disimpan!");
                this.successMessage = "✅ Bandar berjaya disimpan!";
        ...
        async kemaskini({ row }) {
            ...
            let id_lkp_negeri = row.fields[2].value;
            const negeriOption = this.negeriOptions.find(n => n.label === id_lkp_negeri);
            if (negeriOption) {
                id_lkp_negeri = negeriOption.value;
            }

            const aktif = row.fields[3].value === 'Aktif' ? 1 : 0;
            try {
                await lookupService.kemaskiniBandar(id, { nama, id_lkp_negeri, aktif });

                // const negeriLabelUpdated = this.negeriOptions.find(n => n.value === response.data.id_lkp_negeri)?.label || '—';
        ...
        async hapus({ id }) {
            ...
            this.confirmAction = async () => {
                try {
                    await lookupService.hapusBandar(id);
        ...
        async openRecoverModal() {
            try {
                const response = await lookupService.getDeletedBandar(); // You must create this endpoint
        ...
        async pulihkan() {
            try {
                ...
                await lookupService.pulihBandar({ ids: this.selectedRecovery });
        ...
        async loadNegeriList() {
            try {
                const rawData = await lookupService.getNegeri();
                this.negeriList = rawData || [];
            } catch (error) {
                console.error("❌ Gagal ambil senarai negeri:", error);
            }
        },
        ...
        async confirmHapusKekal() {
            try {
                await lookupService.hapusKekalBandar({ ids: this.selectedRecovery });
