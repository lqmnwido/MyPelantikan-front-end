...
export default {
    ...
    methods: {
        async loadJawatan() {
            try {
                ...
                this.jawatanList = response.filter(item => item.id_lkp_jenis_pelantikan === 6).sort((a, b) => a.nama.localeCompare(b.nama));
        ...
        async loadJenisPelantikanID() {
            try {
                ...
                this.id_jenis_pelantikan = 6 || null;
        ...
        async handleFormSubmit() {
            try {
                ...
                formData.append('kategori', 'Keanggotaan Suruhanjaya Perkhidmatan Awam');
                ...
                if (this.isEditMode === 'update' && id_pelantikan) {
                    await suruhanjayaService.kemaskiniAhliSPA(id_pelantikan, formData);
                    this.alert.kemaskini = true;
                } else {
                    await suruhanjayaService.lantikAhliSPA(formData);
                    this.alert.success = true;
                }

                this.$router.push({ path: "/rekod-pelantikan/senarai-pelantikan/kategori/suruhanjaya", query: { senarai: "keanggotaan-suruhanjaya-perkhidmatan-awam" } });
            } catch (error) {
        ...
        async loadAhli(id) {
            try {
                const res = await suruhanjayaService.maklumatAhliSPA(id);
    ...
    computed: {
        pageTitle() {
            return this.isEditMode === 'update' ? 'Kemaskini Suruhanjaya Perkhidmatan Awam' : 'Lantik Suruhanjaya Perkhidmatan Awam';
        },
    }
