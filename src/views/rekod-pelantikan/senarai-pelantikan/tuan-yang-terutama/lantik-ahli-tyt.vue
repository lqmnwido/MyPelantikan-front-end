...
export default {
    ...
    data() {
        return {
            ...
            leftTab4: [
                {
                    label: 'Negeri Lantik', type: 'select-search', placeholder: '- PILIH NEGERI -', model: 'wakil', options: [
                        { label: 'Melaka', value: 4 },
                        { label: 'Sabah', value: 10 },
                        { label: 'Sarawak', value: 11 },
                        { label: 'Pulau Pinang', value: 9 },
                    ],
                    required: true
                },
                {
    ...
    methods: {
        async loadJawatan() {
            try {
                ...
                this.jawatanList = response.filter(item => item.id_lkp_jenis_pelantikan === 11).sort((a, b) => a.nama.localeCompare(b.nama));
        ...
        async loadJenisPelantikanID() {
            try {
                ...
                this.id_jenis_pelantikan = 11 || null;
        ...
        async handleFormSubmit() {
            try {
                ...
                formData.append('kategori', 'Tuan Yang Terutama Yang Di-Pertua Negeri');
                ...
                if (this.isEditMode === 'update' && id_pelantikan) {
                    await tytService.kemaskiniAhliTYT(id_pelantikan, formData);
                    this.alert.kemaskini = true;
                } else {
                    await tytService.lantikAhliTYT(formData);
                    this.alert.success = true;
                }

                this.$router.push({ path: "/rekod-pelantikan/senarai-pelantikan/kategori/tuan-yang-terutama", query: {senarai:"tuan-yang-terutama-yang-di%pertua-negeri"}});
            } catch (error) {
        ...
        async loadAhli(id) {
            try {
                const res = await tytService.maklumatAhliTYT(id);
    ...
    computed: {
        pageTitle() {
            return this.isEditMode === 'update' ? 'Kemaskini Tuan Yang Terutama Di-Pertua Negeri' : 'Lantik Tuan Yang Terutama Di-Pertua Negeri';
        },
    }
