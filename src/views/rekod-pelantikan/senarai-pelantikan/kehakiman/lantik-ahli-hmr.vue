...
export default {
    ...
    methods: {
        async loadJenisPelantikanID() {
            try {
                ...
                this.id_jenis_pelantikan = 2 || null;
        ...
        async handleFormSubmit() {
            try {
                ...
                formData.append('jawatan', data.jawatan ?? 'Hakim Mahkamah Rayuan');
                formData.append('catatan', data.catatan);

                // console.log(data.jawatan ?? 'hakim mahkamah rayuna');
                ...
                if (this.isEditMode === 'update' && id_pelantikan) {
                    await kehakimanService.kemaskiniAhliHMR(id_pelantikan, formData);
                    this.alert.kemaskini = true;
                } else {
                    await kehakimanService.lantikAhliHMR(formData);
                    this.alert.success = true;
                }
            
                this.$router.push({ path: "/rekod-pelantikan/senarai-pelantikan/kategori/kehakiman", query: { senarai: "hakim-mahkamah-rayuan" } });
            } catch (error) {
        ...
        async loadAhli(id) {
            try {
                const res = await kehakimanService.maklumatAhliHMR(id);
    ...
    computed: {
        pageTitle() {
            return this.isEditMode === 'update' ? 'Kemaskini Hakim Mahkamah Rayuan' : 'Lantik Hakim Mahkamah Rayuan';
        },
    }
